...
def initialize_parameters():
    W1 = tf.get_variable("W1", [70, len(features)], initializer = tf.contrib.layers.xavier_initializer())
    b1 = tf.get_variable("b1", [70, 1], initializer = tf.zeros_initializer())
    W2 = tf.get_variable("W2", [35, 70], initializer = tf.contrib.layers.xavier_initializer())
    b2 = tf.get_variable("b2", [35, 1], initializer = tf.zeros_initializer())
    W3 = tf.get_variable("W3", [1, 35], initializer = tf.contrib.layers.xavier_initializer())
...
def model(X_train, Y_train, x_validation, y_validation, keep_prob, learning_rate, num_epochs, minibatch_size, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            val_costs.append(sess.run(cost, feed_dict={X:x_validation, Y:y_validation}))
